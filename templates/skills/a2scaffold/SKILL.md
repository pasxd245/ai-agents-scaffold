...
Do **not** activate it to answer questions about what is *inside* `.agents/`.
...
**Before ever passing `--force`**: it overwrites `.agents/` files *wholesale*,
...
skill does without saying *when* to use it.
...
| Hand edits to a stub vanished | Edits were *inside* the managed region | Move them below `<!-- a2scaffold:end -->`; recover from git |
