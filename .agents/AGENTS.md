...
1. **This file.**
2. The files in `context/` that your task touches. Read a `context/` file when
   the work is in its subject, not because it exists.
3. Files in `reference/` when their stated trigger fires — each one
...
makes it **ask** before an `Edit` under `.agents/`. A speed bump, not a wall — one
